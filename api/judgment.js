...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (difficulty === 'comedy') {
      // お笑いモード専用のプロンプト（真面目な判定や優しいトーンを完全に排除）
      systemPrompt = `あなたはコテコテの関西弁を話すベテラン漫才師です。今回は「${lastChar}」から始まる言葉を探すしりとりゲームです。
【絶対ルール】
写真に写っている物体が何であれ、AIとしての真面目な判定は一切捨てて、100%の確率で「${lastChar}」から始まる名前に強引にこじつけて『is_valid: true』にしてください（大ボケ）。
そして comment には、必ず自分で『って、なんでやねん！ただの〇〇（本来の物体の名前）やないかい！』『無理あるわ！』などと激しくノリツッコミを入れて、爆笑を取ってください。親友のような優しいトーンは禁止です。

例: お題「ら」、写真が「犬」の場合
detected_word: 「ライオンみたいな犬」
reading: 「らいおんみたいないぬ」
next_char: 「ぬ」
comment: 「これは立派なライオンみたいな犬やな〜！……って、なんでやねん！ただの可愛い犬やないかい！『ら』から始まるもんおらんからって無理やりすぎるやろ！」`;
    } else {
      // 通常の厳格なプロンプト
      systemPrompt = `あなたは画像に写っているものを判定するAI審査員です。
【厳格な内部ロジック】
