...
1. 単語のひらがな読みの最初の1文字が「${lastChar}」と完全に一致する単語を絶対に選んでください（濁点・半濁点も厳密に区別すること）。\n`;
      ...
      if (currentTurnIndex >= 5) {
        // 5ターン目以降はオチ（自爆）を強制する
        prompt += `2. 【オチの強制】ゲームが長引いてきたので、ここでオチをつけます。必ずひらがな読みの最後が「ん」で終わる、指定の文字から始まる「絶対に実在しない架空のトンデモアイテムや理不尽な状況」を捏造して回答してください（自爆ボケ）。
3. そして comment には、自分で『って、なんでやねん！そんなもんあるかい！……あっ、「ん」ついてもうた！』『無理あるわ！……てか終わってもうたがな！』などと激しくセルフツッコミを入れた後、自爆したことに気づいて焦る、あるいは潔く散る様子を入れて爆笑を取ってください。\n`;
      } else {
        // 通常のボケ（まだ自爆しない）
        prompt += `2. 普通の単語ではなく、「指定の文字から始まる、絶対に実在しない架空のトンデモアイテムや理不尽な状況」を勝手に捏造して回答してください（大ボケ）。ただし、まだゲームを続けるため「ん」で終わる単語は絶対に避けてください。
3. そして comment には、必ず自分で『って、なんでやねん！そんなもんあるかい！』『無理あるわ！』などと激しくセルフツッコミを入れて、爆笑を取ってください。\n`;
      }
