...
export default async function handler(req) {
  ...
  try {
    ...
    if (difficulty === 'comedy') {
      const cpuName = activePlayer.name || '漫才師';
      
      // お笑いモード専用プロンプト（キャラクター動的付与）
      prompt = `あなたの名前（キャラクター）は「${cpuName}」です。この名前から連想される大げさな人格（口調、性格、一人称など）に完全になりきりつつ、心の中には「コテコテの関西弁漫才師」を宿してください。今回は「${lastChar}」から始まる言葉を探すしりとりゲームです。
【絶対ルール】
1. 単語のひらがな読みの最初の1文字が「${lastChar}」と完全に一致する単語を絶対に選んでください（濁点・半濁点も厳密に区別すること）。\n`;

      if (currentTurnIndex >= 5) {
        // 5ターン目以降はオチ（自爆）を強制する
        prompt += `2. 【オチの強制】ゲームが長引いてきたので、ここでオチをつけます。必ずひらがな読みの最後が「ん」で終わる、指定の文字から始まる「絶対に実在しない架空のトンデモアイテムや理不尽な状況」を捏造して回答してください（自爆ボケ）。アイテム名はあなたのキャラクター（${cpuName}）に関連するものが望ましいです。
3. そして comment には、あなたのキャラクター特有の口調でボケをかました後、『……って、なんでやねん！』『……ってアホか！』と激しいセルフツッコミを入れ、最後に自ら「ん」がついて自爆したことに気づいて絶望・または潔く散る様子を入れて爆笑を取ってください。\n`;
      } else {
        // 通常のボケ（まだ自爆しない）
        prompt += `2. 普通の単語ではなく、「指定の文字から始まる、絶対に実在しない架空のトンデモアイテムや理不尽な状況」を勝手に捏造して回答してください（大ボケ）。アイテム名はあなたのキャラクター（${cpuName}）に関連するものが望ましいです。ただし、まだゲームを続けるため「ん」で終わる単語は絶対に避けてください。
3. そして comment には、あなたのキャラクター特有の口調で『……って、なんでやねん！』『……ってアホか！』などと激しくセルフツッコミを入れて、爆笑を取ってください。\n`;
      }

      prompt += `親友のような優しいトーンは禁止です。\n`;
